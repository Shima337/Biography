...
Система вычисляет следующие метрики для Pipeline v2:
...
3. **Предложения по улучшению** - конкретные рекомендации на основе найденных проблем
...
**Примечание:** Pipeline v1 был удален, так как показывал 0% recall. Используется только Pipeline v2 (двухэтапный: person_extractor → memory_extractor).

## Рекомендации

1. **Используется только Pipeline v2** - показывает отличные результаты (91.67% precision/recall)
